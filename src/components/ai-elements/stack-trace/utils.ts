...
function parseStackFrame(line: string): StackFrame {
  ...
  if (withParensMatch) {
    ...
    const lineNum = withParensMatch[3] ?? null;
    const colNum = withParensMatch[4] ?? null;
    const isInternal
  ...
  if (withoutFnMatch) {
    const filePath = withoutFnMatch[1] ?? null;
    const lineNum = withoutFnMatch[2] ?? null;
    const colNum = withoutFnMatch[3] ?? null;
    const isInternal
      = (filePath?.includes('node_modules') ?? false)
        || (filePath?.startsWith('node:') ?? false)
        || (filePath?.includes('internal/') ?? false);
